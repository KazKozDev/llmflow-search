"""Provides tools for parsing and extracting content from web pages."""
...
def clean_text(text: str) -> str:
    ...
    patterns_to_remove = [
        r"Подписаться на.*", r"Читайте также:.*", r"Поделиться:.*", r"Share.*",
        r"Комментарии.*", r"Copyright ©.*", r"\\d+ комментари(й|ев).*",
        r"Реклама.*", r"Advertisement.*", r"Загрузка комментариев.*",
        r"Популярное:.*", r"По теме:.*", r"Источник:.*",
        r"^\s*Меню\s*$", r"^\s*Поиск\s*$", r"Перейти к содержанию",
