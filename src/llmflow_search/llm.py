...
def _retry_after_bad_json(model: str, messages: list[dict], system: str, content: str, **format_args) -> str:
    try:
        ...
        retry = _ollama_chat(model, retry_messages, tools=None, system=system, temperature=0, **format_args)
...
def _ollama_chat_json(model: str, messages: list[dict], system: str, temperature: float = 0) -> str:
    ...
    return _retry_after_bad_json(model, messages, system, response.get("content", ""), json_mode=True)


def _ollama_chat_schema(model: str, messages: list[dict], system: str, format_schema: dict, temperature: float = 0) -> str:
    ...
    response = _ollama_chat(schema_model, messages, tools=None, system=system, temperature=temperature, format_schema=format_schema)
    return _retry_after_bad_json(schema_model, messages, system, response.get("content", ""), format_schema=format_schema)
