...
def _compact_payload_for_observation(payload: dict, max_chars: int = 4000) -> dict:
    ...
    if "json" in payload:
        compact["json_preview"] = json.dumps(payload.get("json"), ensure_ascii=False)[
            :max_chars
        ]
...
def _normalize_observation(
    ...
    source_type = (
        str(raw.get("source_quality") or raw.get("source_type") or "unknown")
        .lower()
        .strip()
    )
    ...
    publication_dates = (
        ...
        if isinstance(raw.get("publication_dates"), list)
        else []
    )
    ...
    event_dates = (
        ...
        if isinstance(raw.get("event_dates"), list)
        else []
    )
    errors = (
        [
    ...
    return {
        ...
        "source_title": str(
            raw.get("source_title") or raw.get("title") or payload.get("title") or ""
        ).strip()[:200],
...
def _fallback_observation(
    tool_name: str, args: dict, payload: dict, current_step: str
) -> dict:
    ...
    raw = {
        ...
        "publication_dates": [payload.get("pub_date") or payload.get("published")]
        if (payload.get("pub_date") or payload.get("published"))
        else [],
        "event_dates": [],
        "urls": [],
...
OBSERVATION_SCHEMA = {
    ...
    "properties": {
        ...
        "next_action_tags": {"type": "array", "items": {"type": "string"}},
        ...
        "has_rows": {"type": "boolean"},
        "dated": {"type": "boolean"},
        "useful": {"type": "boolean"},
    },
    "required": ["summary", "next_action_tags", "reason", "source_quality", "useful"],
}
...
Diagnose this observation and choose the next search action tags."""
    try:
        role_model = llm.model_for_role(model, "observation")
        with trace.model_call("observation", prompt, role_model):
            content = llm._ollama_chat_schema(
                role_model,
                [{"role": "user", "content": prompt}],
                system=OBSERVATION_SYSTEM_PROMPT,
                format_schema=OBSERVATION_SCHEMA,
            )
        raw = _json_loads_best_effort(content, {})
        if isinstance(raw, dict) and raw:
            observation = _normalize_observation(
                raw, tool_name, args, payload, current_step
            )
            return observation
    except Exception:
        return fallback
...
def _enrich_sources_with_observation(
    sources: list[dict], observation: dict | None
) -> list[dict]:
    if not observation:
        return sources
    enriched = []
    ...
    publication_dates = (
        observation.get("publication_dates")
        if isinstance(observation.get("publication_dates"), list)
        else []
    )
    for source in sources:
        if not isinstance(source, dict):
            ...
            continue
        updated = dict(source)
        if observation.get("source_title") and (
            not updated.get("title") or updated.get("title") == updated.get("url")
        ):
...
def _record_observation(
    memory: dict | None, observation: dict, requirements: dict
) -> dict:
    ...
    compact = {
        ...
        "query_candidates": observation.get(
            "query_candidates", observation.get("suggested_queries", [])
        ),
    ...
    if failure_diagnosis:
        _append_unique(
            memory.setdefault("observation_failure_diagnoses", []), failure_diagnosis
        )
    if observation.get("useful") and observation.get("url"):
        criteria = (
            requirements.get("completion_criteria")
        ...
        key = _slug_key(
            " | ".join(criteria[:2])
            if criteria
            else requirements.get("target", "evidence")
        )
