...
# Reports can be written in Russian, Spanish, or other languages. Embed a broad
# Unicode font instead of silently dropping glyphs through a base-14 PDF font.
...
def _build_style() -> str:
    ...
    if unicode_font:
        font_face_rules.append(
            f'@font-face {{ font-family: "ReportBody"; src: url("{unicode_font}"); }}'
        )
        if bold_font:
            font_face_rules.append(
...
def _extract_title(report_markdown: str, fallback: str) -> tuple[str, str]:
    """Extract a leading Markdown H1 for the styled document title."""
...
def render_report_pdf(
    *,
    ...
    """Render a completed report's Markdown as a letterhead-style PDF."""
    ...
    if logo_path is not None:
        ...
        if data_uri:
            logo_html = f'<img src="{data_uri}" alt="LLMFlow-Search" />'
...
<div class="doc-meta">LLMFlow-Search Research Report &bull; Generated {generated_at} &bull; ID: {html.escape(research_id)}</div>
<hr class="top-rule" />
{body_html}
<div id="footer_content">
  <span>{html.escape(_truncate_at_word_boundary(title, 80))}</span> -
  <span>Page <pdf:pagenumber /> of <pdf:pagecount /></span>
    ...
    error_count = getattr(result, "err", 0)
    if error_count:
        raise RuntimeError(f"PDF generation failed with {error_count} error(s)")
    return output_path
