...
def _kwargs_from_schema(raw_arg: str, properties: dict) -> dict | None:
    ...
    names = "|".join(re.escape(name) for name in sorted(properties, key=len, reverse=True))
    ...
    for index, match in enumerate(matches):
        ...
        if not value:
            return None
        kwargs[match.group(1)] = _coerce_to_schema(value, properties.get(match.group(1)) or {})
...
def _tool_call_from_schema_step(step: str, tools: list[dict]) -> dict | None:
    ...
    if isinstance(parsed, dict):
        ...
        required = [
            item for item in (raw_required if isinstance(raw_required, list) else [])
            if item in properties
        ]
        candidates = required if len(required) == 1 else list(properties) if len(properties) == 1 else []
...
def _step_identity(step: str, tools: list[dict] | None = None) -> str:
    ...
    raw_required = schema.get("required")
    required = [item for item in (raw_required if isinstance(raw_required, list) else []) if item in arguments]
    keys = required or sorted(arguments)
    signature = " ".join(f"{key}={' '.join(str(arguments[key]).split())}" for key in sorted(keys))
