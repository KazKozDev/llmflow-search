...
def select_profile(tool_names, env: str | None = None) -> Profile:
    ...
    choice = (env if env is not None else os.getenv("LLMFLOW_SEARCH_PROFILE", "auto")).strip().lower()
    if choice == "footnote":
        return FOOTNOTE_PROFILE
    if choice == "generic":
        return GENERIC_PROFILE
    return FOOTNOTE_PROFILE if FOOTNOTE_SIGNATURE <= set(tool_names) else GENERIC_PROFILE
