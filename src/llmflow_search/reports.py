...
def _build_debug_report(state: dict) -> dict:
    ...
    try:
        from footnote_mcp.tools_data import build_research_debug_report  # pyright: ignore[reportMissingImports]
    except ImportError:
...
def _write_debug_report(state: dict) -> str | None:
    if os.getenv("LLMFLOW_SEARCH_DEBUG_REPORTS", "0") != "1":
        return None
    output_dir = Path(os.getenv("LLMFLOW_SEARCH_DEBUG_REPORT_DIR", "~/.llmflow-search/debug_reports")).expanduser()
...
def _report_markdown(state: dict) -> str:
    ...
    if sources:
        ...
        for index, source in enumerate(sources, 1):
            ...
            lines.append(f"{index}. [{title}]({url}) - {url}" if url else f"{index}. {title}")
    return "\n".join(lines).strip() + "\n"
...
def _write_pdf_report(state: dict) -> str | None:
    ...
    output_path = Path(PDF_REPORTS_DIR).expanduser() / report_pdf_filename(research_id, task)
