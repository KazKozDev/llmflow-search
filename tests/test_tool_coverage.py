...
def test_papers_search_records_with_abstracts_become_sources():
    ...
    # The abstract is labelled as index metadata so the ledger cannot read it as full text.
    assert sources[0]["content"].startswith("[arxiv index record")
    assert "cross-entropy loss" in sources[0]["content"]
...
def test_a_fetched_archive_record_is_not_labelled_as_index_metadata():
    ...
    content = _sources_from_tool_result("archive_search", payload)[0]["content"]

    assert content.startswith("Annual figures")
...
def test_github_and_encyclopedia_records_carry_their_own_kind():
    ...
    assert _sources_from_tool_result("encyclopedia_search", entity)[0]["kind"] == "encyclopedia"
...
def test_web_crawl_yields_one_source_per_readable_page():
    payload = json.dumps({
        ...
        "pages_crawled": 3,
        "pages": [
            {"url": "https://example.com/docs", "title": "Docs", "text": "Index of the docs."},
            {"url": "https://example.com/docs/a", "title": "A", "text": "Page A body."},
            {"url": "https://example.com/docs/b", "error": "HTTP 404", "text_length": 0},
        ],
    })
...
def test_date_ranged_browser_tables_become_a_source_with_their_range():
    payload = json.dumps({
        "date_range": {"start_date": "2026-07-01", "end_date": "2026-07-31", "submitted": True},
        "url": "https://bank.example/history",
        "title": "History",
        "tables": [{"columns": ["date", "rate"], "rows": [{"date": "2026-07-01", "rate": "90.8"}]}],
        "table_count": 1,
    })

    sources = _sources_from_tool_result("browser_extract_tables_for_date_range", payload)
...
def test_detected_download_links_survive_the_invented_url_gate():
    memory = _update_search_memory(
        ...
        json.dumps({
            ...
            "count": 1,
            "downloads": [
                {"url": "https://gov.example/data/2026.csv", "text": "2026 dataset", "extension": ".csv"}
            ],
        }),
    )
...
def test_subject_index_results_are_registered_as_discovered_urls():
    memory = _update_search_memory(
        ...
        json.dumps({
            "count": 1,
            "results": [{"title": "Scaling Laws", "url": "https://arxiv.org/abs/2001.08361"}],
        }),
    )

    assert memory["discovered_urls"] == ["https://arxiv.org/abs/2001.08361"]
    assert memory["discovered_titles"]["https://arxiv.org/abs/2001.08361"] == "Scaling Laws"
    ...
    archive = _update_search_memory(
        ...
        json.dumps({"count": 1, "results": [{"url": "https://web.archive.org/web/2020/gone"}]}),
    )
...
def test_strategy_plan_keeps_a_changed_access_path_and_defaults_to_web_search():
    memory = _merge_search_memory(
        {"next_queries": ["papers_search: transformer scaling laws", "euro rate august 2026"]}
    )
...
def test_a_subject_index_step_runs_and_reaches_the_ledger_as_a_source(monkeypatch):
    ...
    monkeypatch.setattr(
        nodes_module, "_ollama_chat_json",
    ...
    update = asyncio.run(execute_node(state, "main", [_PAPERS_SEARCH_TOOL], FOOTNOTE_PROFILE))

    # Resolved from the live schema — no hard-coded parser entry for this tool.
    assert calls == [("papers_search", {"query": "scaling laws for neural language models"})]
    assert [source["url"] for source in update["candidate_sources"]] == [
        "https://arxiv.org/abs/2001.08361"
    ]
    assert update["search_memory"]["discovered_urls"] == ["https://arxiv.org/abs/2001.08361"]
...
def test_plain_value_and_json_object_forms_still_work():
    ...
    url = _tool_call_from_schema_step("web_read: https://example.com/a?x=1", _PARSE_TOOLS)
...
def test_a_search_operator_is_a_query_not_a_named_argument():
    ...
    for step in ("web_search: site=example.com spain news", "web_search: site:example.com spain news"):
...
def test_a_bare_url_is_never_mistaken_for_named_arguments():
    ...
    call = _tool_call_from_schema_step("web_read: https://example.com/a?x=1", _PARSE_TOOLS)
...
def test_an_unquoted_multi_word_value_survives_intact():
    call = _tool_call_from_schema_step("web_search: query: python 3.14 release notes", _PARSE_TOOLS)
...
def test_the_same_call_written_two_ways_has_one_identity():
    ...
    named = _step_identity("web_search: query: 'best local LLM August 2026' num:10", _PARSE_TOOLS)
    json_form = _step_identity(
        'web_search: {"query": "best local LLM August 2026", "lang": "en"}', _PARSE_TOOLS
    )
...
_PACING_TOOLS = [
    _tool(
        "web_search",
        {"query": {"type": "string"}, "lang": {"type": "string"}, "num": {"type": "integer"}},
        ["query"],
    ),
...
def test_a_long_search_batch_is_spread_across_rounds(monkeypatch):
    ...
    monkeypatch.setattr(llm, "_ollama_chat", lambda *a, **k: {"content": ""})
    monkeypatch.setattr(nodes_module, "MAX_THROTTLED_STEPS_PER_BATCH", 3)

    plan = [f"web_search: query {i}" for i in range(6)]
    update = asyncio.run(execute_node(_throttled_state(plan), "main", _PACING_TOOLS, FOOTNOTE_PROFILE))
...
def test_every_throttled_family_runs_one_at_a_time(monkeypatch):
    ...
    asyncio.run(execute_node(_throttled_state(plan), "main", _PACING_TOOLS, FOOTNOTE_PROFILE))
...
def test_the_search_budget_stops_a_question_from_draining_the_quota(monkeypatch):
    ...
    monkeypatch.setattr(nodes_module, "MAX_THROTTLED_STEPS_PER_BATCH", 2)
