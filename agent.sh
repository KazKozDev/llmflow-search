#!/usr/bin/env bash
# Launch the LLMFlow-Search research agent on Linux. Syncs its locked uv environment,
# installs the sibling footnote-mcp server package, ensures Chromium, then runs the agent.
set -euo pipefail

PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$PROJECT_DIR"

print_logo() {
  local blue=$'\033[38;2;103;148;254m'
  local flow_blue=$'\033[38;2;81;115;205m'
  local search_gray=$'\033[38;2;174;181;193m'
  local reset=$'\033[0m'

  printf '%s\n' "

${blue}
██╗     ██╗     ███╗   ███╗ ${flow_blue}███████╗██╗      ██████╗ ██╗    ██╗
██║     ██║     ████╗ ████║ ${flow_blue}██╔════╝██║     ██╔═══██╗██║    ██║
██║     ██║     ██╔████╔██║ ${flow_blue}█████╗  ██║     ██║   ██║██║ █╗ ██║
██║     ██║     ██║╚██╔╝██║ ${flow_blue}██╔══╝  ██║     ██║   ██║██║███╗██║
███████╗███████╗██║ ╚═╝ ██║ ${flow_blue}██║     ███████╗╚██████╔╝╚███╔███╔╝
╚══════╝╚══════╝╚═╝     ╚═╝ ${flow_blue}╚═╝     ╚══════╝ ╚═════╝  ╚══╝╚══╝${reset}

${search_gray}███████╗ ███████╗  █████╗  ██████╗   ██████╗ ██╗  ██╗
██╔════╝ ██╔════╝ ██╔══██╗ ██╔══██╗ ██╔════╝ ██║  ██║
███████╗ █████╗   ███████║ ██████╔╝ ██║      ███████║
╚════██║ ██╔══╝   ██╔══██║ ██╔══██╗ ██║      ██╔══██║
███████║ ███████╗ ██║  ██║ ██║  ██║ ╚██████╗ ██║  ██║
╚══════╝ ╚══════╝ ╚═╝  ╚═╝ ╚═╝  ╚═╝  ╚═════╝ ╚═╝  ╚═╝${reset}



"
}

print_logo

if ! command -v uv >/dev/null 2>&1; then
  echo "Installing uv..."
  curl --proto '=https' --tlsv1.2 -LsSf https://astral.sh/uv/install.sh | sh
  export PATH="$HOME/.local/bin:$PATH"
fi

if [[ ! -d ../footnote-mcp ]]; then
  if ! command -v git >/dev/null 2>&1; then
    echo "[!] git is required to fetch footnote-mcp. Install git, or clone"
    echo "    https://github.com/KazKozDev/footnote-mcp.git into $(dirname "$PROJECT_DIR") yourself."
    exit 1
  fi
  echo "footnote-mcp not found next to this checkout — cloning it..."
  git clone --quiet https://github.com/KazKozDev/footnote-mcp.git ../footnote-mcp
fi

if ! command -v ollama >/dev/null 2>&1; then
  echo "[!] Ollama was not found on PATH. Install it from https://ollama.com and"
  echo "    pull at least one model (e.g. \`ollama pull qwen2.5:7b\`) before continuing."
fi

uv python install --quiet
uv sync --locked --quiet
uv pip install --quiet --python .venv/bin/python -e ../footnote-mcp "mcp<2"
uv run --no-sync python -m playwright install chromium >/dev/null 2>&1 || true

# Local secrets (API keys) live in .env, which is gitignored. Keeping them here
# rather than in the shell profile keeps them out of shell history too.
if [[ -f .env ]]; then
  set -a
  # shellcheck disable=SC1091
  source .env
  set +a
fi

export LLMFLOW_SEARCH_FORCE_COLOR="${LLMFLOW_SEARCH_FORCE_COLOR:-1}"
uv run --no-sync python -m llmflow_search

echo ""
read -r -p "Press Enter to close..." _ || true
